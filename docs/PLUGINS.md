...
> 보존된 실험 구현입니다. 현재 주력은 [스킬 + 스타일 파일 + 조회용 MCP](STYLE_REBOOT.md)입니다.

이 실험에서는 편집기 내부 UI와 화면 없는 Python 로컬 서비스를 구현했습니다.
아래 실행법은 유지하지만 주력 스킬 경로에서 이 엔진이나 패널을 설치할 필요는 없습니다.
