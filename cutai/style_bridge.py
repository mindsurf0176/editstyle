...
PRESETS = Path(__file__).resolve().parent.parent / "awesome-editstyles" / "presets"
...
def read_style(markdown: str) -> dict:
    ...
    for line in markdown.splitlines():
        ...
        if current is not None:
            current["lines"].append(line)
    if not marker:
        raise ValueError("Missing blockquote marker: > CutAI EDITSTYLE v1")
    if not name:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CutAI portable editing styles (no rendering)")
